    ...
    return (
      ...
      >
         <div className="absolute inset-0 z-10 flex flex-col p-8 md:p-14 justify-end bg-gradient-to-t from-black/50 via-black/10 to-transparent text-white">
           <span className="text-[11px] font-semibold uppercase tracking-widest text-white/80 mb-3">{product.category}</span>
           <h3 className="text-4xl md:text-5xl font-semibold mb-3 tracking-tight text-white">{product.title}</h3>
           <p className="text-lg md:text-xl text-white/90 max-w-lg leading-snug">{product.tagline}</p>
         </div>
         <img 
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-7 flex flex-col justify-between">
        <div>
           <span className="text-[10px] font-bold uppercase tracking-wider text-blue-600 mb-2 block">{product.category}</span>
           <h3 className="text-[19px] font-semibold text-[#1D1D1F] mb-2 tracking-tight">{product.title}</h3>
           <p className="text-[13px] leading-relaxed text-[#86868b] line-clamp-2">{product.tagline}</p>
        </div>
        <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
            <span className="text-[11px] text-gray-400 font-medium">{new Date(product.date).getFullYear()}</span>
            <span className="text-[11px] font-medium text-[#0066CC] group-hover:underline">View Details</span>
        </div>
      </div>
    </Link>
  );
