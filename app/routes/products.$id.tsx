...
import { loadProduct } from "../products.server";
...
export function meta({ data }: Route.MetaArgs) {
  ...
  return [
    { title: `${data.product.title} - Lumina` },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-16">
              <div>
                <h3 className="text-2xl font-semibold mb-4 text-[#1D1D1F] tracking-tight">
                  Overview
                </h3>
                {/* Markdown Content */}
                <div
              ...
              {/* Gallery */}
              <div>
                <h3 className="text-2xl font-semibold mb-6 text-[#1D1D1F] tracking-tight">
                  Gallery
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {product.galleryImages.map((img, idx) => (
                    <div
                      key={idx}
                      className="rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-lg transition-shadow duration-300 ring-1 ring-gray-100"
                    >
                      <img
                        src={img}
                        alt={`Gallery ${idx}`}
            ...
            <div className="lg:col-span-4 space-y-8">
              <div className="bg-white p-8 rounded-[24px] shadow-sm ring-1 ring-gray-100/50 sticky top-20">
                <h3 className="text-[15px] font-semibold mb-6 text-[#1D1D1F]">
                  Specifications
                </h3>
                ...
                <div className="space-y-4">
                  ...
                  {product.attributes.map((attr, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-baseline py-2 border-b border-gray-50 last:border-0"
                    >
                      <span className="text-[#86868b] text-[13px]">{attr.label}</span>
                      <span className="font-medium text-[#1D1D1F] text-[13px] text-right">
                        {attr.value}
                      </span>
                    </div>
                  ))}
      ...
      <div className="bg-[#1D1D1F] text-white py-24 text-center px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 text-lg mb-10">
            Access {product.title} securely with your corporate credentials.
          </p>
          <a
            href={product.link}
